...
请输出："""
    ...
    async def generate_image(self, content: str, sharing_type: SharingType, life_context: str = None) -> Optional[str]:
        ...
        # 兜底：自动模式下 sharing_type 可能为 None
        if sharing_type is None:
            sharing_type = SharingType.GREETING
        ...
        # 2. 检测是否启用 Gitee 参考图逻辑（所有生图模式都使用参考图）
        use_gitee_ref = self.img_conf.get("use_gitee_selfie_ref", False)

        logger.info(f"[DailySharing] 配图决策: {mode_str} ({logic_str}) | 类型: {sharing_type.value} | 参考图模式: {use_gitee_ref}")        
    ...
    async def _assemble_final_prompt(self, content: str, sharing_type: SharingType, involves_self: bool, visuals: Dict) -> str:
        ...
        # 定义质量词
        quality_tags = "8K分辨率, 高质量, 高分辨率, 细节丰富, 色彩鲜艳, 电影级光影效果"
    ...
    async def generate_video_from_image(self, image_path: str, content: str, is_scheduled: bool = True) -> Optional[str]:
        ...
        try:
            ...
            source = "聊天触发" if not is_scheduled else "定时分享"
            logger.info(f"[DailySharing] 视频参考图选择模式: {source}")

            if is_scheduled:
                # 定时分享：优先 AI 配图 → 自拍参考图 → 外部目录
                if not image_bytes and os.path.exists(image_path):
                    with open(image_path, "rb") as f: image_bytes = f.read()
                    logger.info(f"[DailySharing] [分享] 使用 AI 配图生成视频")
                ...
                if not image_bytes:
                    ...
                    if use_selfie:
                        ...
                        if ref_images:
                            ...
                            logger.info(f"[DailySharing] [分享] 使用自拍参考图生成视频 ({len(image_bytes)} bytes)")
            ...
            # 人物音色
            video_voice = self.img_conf.get("video_voice_hint", "").strip()
            if video_voice:
                parts.append(video_voice)
            ...
            # 取第一个可用的提供商 ID
            provider_id = chain[0]
            logger.info(f"[DailySharing] 视频提供商链: {chain}, 使用: {provider_id}")
            try:
    ...
    @staticmethod
    def _load_latest_image_from_dir(dir_path: str) -> Optional[bytes]:
        """从指定目录加载最新（修改时间最新）的图片文件"""
        ...
        try:
            ...
            if not candidates:
                logger.warning(f"[DailySharing] 视频参考图目录为空: {dir_path}")
                return None
            latest = max(candidates, key=os.path.getmtime)
            with open(latest, "rb") as f:
                data = f.read()
            logger.info(f"[DailySharing] 选取最新图片: {os.path.basename(latest)}")
            return data
        except Exception as e:
            logger.warning(f"[DailySharing] 读取视频参考图目录失败: {e}")
            return None

    @staticmethod
    def _load_latest_images_from_dir(dir_path: str, count: int = 2) -> List[bytes]:
        """从指定目录加载最新的 N 张图片（按修改时间倒序）"""
        supported_exts = {".png", ".jpg", ".jpeg", ".webp", ".bmp"}
        try:
            candidates = [
                os.path.join(dir_path, f)
            ...
            for path in candidates[:count]:
                ...
                logger.info(f"[DailySharing] 选取图片: {os.path.basename(path)}")
            return result
        except Exception as e:
            logger.warning(f"[DailySharing] 读取视频参考图目录失败: {e}")
            return []
    ...
    async def _call_aiimg(self, prompt: str, use_ref: bool = False, involves_self: bool = False) -> Optional[str]:
        ...
        try:
            # ================= 参考图逻辑 =================
            if use_ref and hasattr(self._aiimg_plugin, "edit"):
                logger.info("[DailySharing] 尝试使用参考图生成...")
                ...
                if ref_images:
                    ...
                    if involves_self:
                        ...
                        final_prompt = (
                            "请根据参考图的整体风格生成一张新的图片：\n"
                            "1) 保持参考图的画风和艺术风格（如二次元、写实、插画等），不要改变风格。\n"
                            f"2) 画面具体描述：{prompt}\n"
                            "3) 不要拼图，不要水印。"
                        )
                        task_types = ["background", "style"]
                    ...
                    logger.warning("[DailySharing] 已开启参考图模式，但未找到参考图，降级为文生图")
