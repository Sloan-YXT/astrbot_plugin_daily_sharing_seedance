...
class TaskManager:
    ...
    async def send(self, uid, text, img_path, audio_path=None, video_url=None):
        ...
        try:
            ...
            # 3. 分享视觉媒体（视频优先，其次图片）
            if video_url:
                # 如果是远程 URL，先下载到本地缓存再发送（避免 QQ 协议端无法访问外部 URL）
                local_video_path = video_url
                if video_url.startswith("http"):
                    try:
                        import aiohttp, hashlib
                        video_cache_dir = os.path.join(
                            os.path.dirname(os.path.dirname(__file__)), "data", "cache", "video"
                        )
                        os.makedirs(video_cache_dir, exist_ok=True)
                        url_hash = hashlib.md5(video_url.encode()).hexdigest()[:12]
                        local_video_path = os.path.join(video_cache_dir, f"{url_hash}.mp4")

                        async with aiohttp.ClientSession() as sess:
                            async with sess.get(video_url, timeout=aiohttp.ClientTimeout(total=120)) as resp:
                        ...
                        logger.info(f"[DailySharing] 视频已缓存到本地: {local_video_path} ({file_size_mb:.1f}MB)")
                    except Exception as e:
                        logger.warning(f"[DailySharing] 视频下载到本地失败: {e}，尝试直接用 URL 发送")
                        local_video_path = video_url

                video_chain = MessageChain()
                if local_video_path.startswith("http"):
                    video_chain.chain.append(Video.fromURL(local_video_path))
                else:
                    video_chain.chain.append(Video.fromFileSystem(local_video_path))
