...
class DailySharingPlugin(Star):
    ...
    async def daily_share_tool(
        ...
        """
        日常分享/自拍/配图/视频/新闻/热搜/语音 —— 万能内容生成工具。

        【触发规则 - 必须严格遵守】
        只要用户消息中出现以下任意关键词或意图，就必须调用本工具：
        - 自拍、拍照、拍一张、来张照片、发个照片
        - 视频、动态图、动起来、动图、录一段
        - 分享、说说、发个说说、发空间
        - 新闻、热搜、今日头条、60s
        ...
        即使用户用口语化/撒娇/命令式语气（如"宝宝来个XX"、"亲爱的给我XX"、"帮我整个XX"），
        只要包含上述关键词就必须调用本工具，不要当作普通聊天回复。

        【重要】本工具是唯一支持视频生成的工具。aiimg_generate 只能生成静态图片，不支持视频。
        当用户提到"视频"二字时，必须使用本工具并设置 need_video=True 和 need_image=True。

        Args:
            share_type(string): 分享类型。支持：'自动', '问候', '新闻', '心情', '知识', '推荐', '60s新闻', 'AI资讯'。当用户没有明确指出发什么类型的内容(比如只说"发个说说"、"分享一下"、"发个自拍"、"来个自拍视频")时，请务必将其设为 '自动'。
            ...
            need_image(boolean): 是否需要AI为这段文案配图。默认为 False。当用户提到"配图"、"带图"、"发张图"、"自拍"、"拍照"、"照片"时，设为 True。
            need_video(boolean): 是否需要AI为这段文案生成视频。默认为 False。当用户消息中包含"视频"、"动态图"、"动起来"、"动图"时，必须设为 True。设为 True 时 need_image 也必须同时设为 True（视频依赖先生成图片）。
    ...
    @filter.regex(r"自拍|视频|拍[一个张]|录[一个]|动态|照片|来[张个一].*[照图拍]")
    async def handle_selfie_intent_check(self, event: AstrMessageEvent):
        """粗筛含相关关键词的消息，用 LLM 精确判断是否需要生成自拍/视频"""
        if self._is_terminated:
            return
        if self._lock.locked():
            return
        ...
        # 立刻阻止事件传播，防止 group_chat_plus 抢先回复
        # 如果 LLM 判定不需要生成，再恢复
        event.stop_event()
        ...
        judge_prompt = (
            f"用户消息：「{user_msg}」\n\n"
            "请判断用户是否在要求bot生成自拍图片或自拍视频。\n"
            "只回复一个JSON，格式：{\"need_image\": true/false, \"need_video\": true/false}\n"
            "- need_image: 用户想要bot自拍/拍照/来张照片/发个自拍（仅图片）\n"
            "- need_video: 用户想要视频/动态图/自拍视频/录一段\n"
            "注意：\n"
            "- 如果用户想要视频，need_image也必须为true（视频依赖图片）\n"
            "- 如果用户只是在闲聊、讨论、提到这些词但不是在请求生成内容，两个都填false\n"
            "- 例如'你昨天拍的自拍好好看'是闲聊，填false；'来个自拍'是请求，填true\n"
            "只回复JSON，不要解释。"
        )
        try:
            result = await self._call_llm_wrapper(judge_prompt, timeout=15, max_retries=0)
            if not result:
                event.continue_event()  # 恢复，让其他插件处理
                return
            ...
            need_image = parsed.get("need_image", False)
            need_video = parsed.get("need_video", False)

            if not need_image and not need_video:
                event.continue_event()  # LLM 判定不需要，恢复传播
                return

            logger.info(f"[DailySharing] LLM 意图判断: need_image={need_image}, need_video={need_video}, 原文: {user_msg[:50]}")

            # 让 LLM 按人格生成一句自然的"稍等"回复
            action = "自拍视频" if need_video else "自拍"
            reply_prompt = (
                f"用户要你{action}，你正在准备中。\n"
                f"请用你的人格和语气，简短回复一句（1句话，不超过30字），"
                f"表达'在弄了/稍等一下'的意思。每次说法要不同，符合当前心情和场景。\n"
                f"不要用引号，直接回复内容。"
            )
            try:
                reply_text = await self._call_llm_wrapper(reply_prompt, timeout=10, max_retries=0)
                if reply_text:
                    await event.send(event.plain_result(reply_text.strip()))
            except Exception:
                pass

            # 保持 stop_event，后台启动生成任务
            task = asyncio.create_task(
                self.task_manager.async_daily_share_task(
                    event, "自动", None, True, need_image, need_video, False, False
                )
            )
            self._bg_tasks.add(task)
            task.add_done_callback(self._bg_tasks.discard)
            ...
            logger.debug(f"[DailySharing] 意图判断失败，跳过: {e}")
            event.continue_event()  # 出错恢复
            return
